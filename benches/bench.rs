...
fn fat_hnsw_construction() {
	...
	.with_insert_heuristic_extend(false)
	;
	let index = HNSWParallelHeapBuilder::<u64,_,_>::build_fat(data.view(), SquaredEuclideanDistance::new(), params, 1);
	let _ = index.knn_query_batch(&queries, 100);
}
fn fat_hnsw_capped_construction() {
	use graphidxbaselines::hnsw::*;
	let (data, queries) = init_data();
	let params = HNSWParams::new()
	.with_insert_heuristic(true)
	...
	.with_max_build_frontier_size(Some(10))
	;
	let index = HNSWParallelHeapBuilder::<u64,_,_>::build_fat_capped(data.view(), SquaredEuclideanDistance::new(), params, 1, 10);
	let _ = index.knn_query_batch(&queries, 100);
}
fn rnn_construction() {
...
fn main() {
	...
	easy_bench!(fat_hnsw_construction);
	easy_bench!(fat_hnsw_capped_construction);
